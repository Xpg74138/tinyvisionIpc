...
*/
    LOGI("cfg->session_count:%d",cfg->session_count);//3
    fclose(fp);
	return count;
}
...
int main(int argc, char *argv[])
{
	...
	for (ch = 0; ch < session_count; ch++) {
		...
		session[ch] = rtsp_new_session(demo, cfg.session_cfg[ch].path,NULL,NULL);//对应rtsp session 
		if (NULL == session[ch]) {
			LOGW("rtsp_new_session failed");
			continue;
		}
		...
		LOGI("==========> rtsp://127.0.0.1:8554%s for %s %s <===========", cfg.session_cfg[ch].path, 
	...
	while (flag_run) {
        ...
        do {
            ret = rtsp_do_event(demo);		//rtsp交互，监听rtsp端口，并把连接上的客户端的套接字分别放进队列中,rtsp_sever_tx_video函数里面也调用这个接口
            if (ret > 0)
                continue;
            if (ret < 0)
                break;
