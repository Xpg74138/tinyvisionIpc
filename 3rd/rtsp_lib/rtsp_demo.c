...
struct rtsp_session			//会话信息结构体
{
	...
	char szUsername[64];	//add by ljh for authenticator!用户密码认证
	char szPassword[64];
	RtspCallbackStatus CallbackStatus;	//状态回调函数用于连接后请求i帧或记录连接的客户端数
...
struct rtsp_demo 
{
	SOCKET sockfd;	//rtsp server socket 0:invalid			//监听流端口会话
	struct rtsp_session_queue_head sessions_qhead;			//发布的流会话队列(流名)
	struct rtsp_client_connection_queue_head connections_qhead;		//连接上的客户端对应的流
};
...
static void rtsp_del_client_connection (struct rtsp_client_connection *cc)
{
	if (cc) {
		#if 0	//用户名出错时，会有bug
		if(cc->session->CallbackStatus)
			cc->session->CallbackStatus(false,cc->session->path);
		#endif
...
rtsp_session_handle rtsp_new_session (rtsp_demo_handle demo, const char *path,char *pUsername,char *pPassword)
{
	...
	memset(s->szUsername,0,sizeof(s->szUsername));
	memset(s->szPassword,0,sizeof(s->szPassword));
	if(pUsername!=NULL)
		memcpy(s->szUsername,pUsername,strlen(pUsername));
	if(pPassword!=NULL)
		memcpy(s->szPassword,pPassword,strlen(pPassword));
	s->CallbackStatus=NULL;
//	printf("==============%s,%s,%s,%s\n",s->szUsername,s->szPassword,pUsername,pPassword);
...
rtsp_session_handle create_rtsp_session(rtsp_demo_handle demo, const char *path,char *pUsername,char *pPassword)
{
    rtsp_session_handle session;
    session = rtsp_new_session(demo,path,pUsername,pPassword);
...
int AuthenticateDigest(struct rtsp_client_connection *cc,rtsp_msg_s *resmsg) //add by ljh for Authenticate
{
	struct rtsp_session *s = cc->session;
	if(strlen(s->szUsername)!=0){
		if(strstr(cc->reqbuf,"username=\"")==NULL){
			rtsp_msg_set_response(resmsg, 401);
			return -1;
		}else{
			int nRet=RtspAuthenticateDigest(cc->reqbuf,s->szUsername,s->szPassword);
			if(0!=nRet){
				rtsp_msg_set_response(resmsg, 401);
				return -1;
			}
...
#if 0
dbg("============s->path=%s,szUsername=%s,szPassword=%s\n",s->path,s->szUsername,s->szPassword);
dbg("=======type=%d,body=%s,cc->reqbuf=[%s]\n",reqmsg->type,reqmsg->body,cc->reqbuf);
if(strlen(s->szUsername)==0){			//add by ljh for Authenticate
	if(strstr(cc->reqbuf,"username=\"")==NULL){
		rtsp_msg_set_response(resmsg, 401);
		return 0;
	}else{
...
//	struct rtsp_demo *d = cc->demo;
	...
	if (cc->state != RTSP_CC_STATE_PLAYING) {
		...
		if(s->CallbackStatus)
            s->CallbackStatus(true,s->path, 0);
	}
	return 0;
}
...
//	struct rtsp_demo *d = cc->demo;
	...
	dbg("=====vpath=%s,apath=%s,reqmsg->hdrs.startline.reqline.uri.abspath=%s,s->path=%s\n", vpath, apath, reqmsg->hdrs.startline.reqline.uri.abspath, s->path);
...
//			dbg("============s->path=%s,path=%s,szUsername=%s,szPassword=%s\n",s->path,path,s->szUsername,s->szPassword);
		}
		if (NULL == s) {
	...
	if (reqmsg->hdrs.startline.reqline.method != RTSP_MSG_METHOD_OPTIONS){
		if(0!=AuthenticateDigest(cc,resmsg))
			return 0;
	}
#endif
...
#if 1	//add by ljh for Authenticate
	if(msg->hdrs.startline.resline.status_code==401){
		char *pData="WWW-Authenticate: Digest realm=\"rtsp_demo\",nonce=\"ab1234c446bfa49b0c48c3edb83139543d\",stale=\"FALSE\"\r\n\r\n";
		char SendBuf[1024] = "";
		memcpy(SendBuf,szbuf, sizeof(szbuf));
		memcpy(SendBuf+strlen(szbuf)-2,pData, strlen(pData));
		int nLen=strlen(pData)+strlen(szbuf)-2;
//		printf("======szbuf=[%s],nLen=%d,strlen=%d,sizeof(szbuf)=%d,strlen(szbuf)=%d,sizeof(pData)=%d\n",SendBuf,nLen,strlen(SendBuf),sizeof(szbuf),strlen(szbuf),sizeof(pData));
		ret = send(cc->sockfd, SendBuf, nLen, 0);
	}else
#endif
	...
	while (cc) {//存在rtsp_client_connection
		...
		if (FD_ISSET(cc1->sockfd, &rfds)) {
			do {
				...
				ret = rtsp_recv_msg(cc1, &reqmsg);		//接收rtsp交互信息
				if (ret == 0)
					break;
				if (ret < 0) {
				...
				ret = rtsp_process_request(cc1, &reqmsg, &resmsg);		//rtsp信息交互交互，OPTIONS，PLAY等
				if (ret < 0) {
...
int rtsp_tx_video (rtsp_session_handle session, const uint8_t *frame, int len, uint64_t ts)
{
	...
	TAILQ_FOREACH(cc, &s->connections_qhead, session_entry) {
		...
		if (!streamq_inused(q, rtp->streamq_index) && rtp->streamq_index != streamq_tail(q)) {
			rtp->streamq_index = streamq_head(q);							//把视频分包的队列句柄赋值给rtp会话的连接流句柄队列
	...
	case RTSP_CODEC_ID_VIDEO_H264:
		if (s->vcodec_data.h264.pps_len == 0 || s->vcodec_data.h264.pps_len == 0) {
			if (rtsp_codec_data_parse_from_frame_h264(frame, len, &s->vcodec_data.h264) < 0) {	//填充sps pps头
...
//					s->vcodec_data.h264.sps_len, s->vcodec_data.h264.pps_len);
	...
	case RTSP_CODEC_ID_VIDEO_H265:
		if (s->vcodec_data.h265.pps_len == 0 || s->vcodec_data.h265.pps_len == 0 || s->vcodec_data.h265.vps_len == 0) {
			if (rtsp_codec_data_parse_from_frame_h265(frame, len, &s->vcodec_data.h265) < 0) {	//填充vps sps pps头
...
//					s->vcodec_data.h265.vps, s->vcodec_data.h265.sps_len, s->vcodec_data.h265.pps_len);
	...
	while (start < len && packets[count] && pktsizs[count] > 0) {
		...
		switch (s->vcodec_id) {
		case RTSP_CODEC_ID_VIDEO_H264:
			ret = rtp_enc_h264(&s->vrtpe, p, size, ts, &packets[count], &pktsizs[count]);	//切包分片，数据分别存到packets[count]，大小为pktsizs[count]
			if (ret <= 0) {
	...
	TAILQ_FOREACH(cc, &s->connections_qhead, session_entry) {
		...
		rtcp_try_tx_sr(rtp, s->video_ntptime_of_zero_ts, ts, s->vrtpe.sample_rate);		//发送rtcp包
		rtsp_tx_video_packet(cc);														//发送rtp包，将切片好的rtp包分别轮训发送给连接上服务器的会话
	}
...
#if 0
void CallbackStatus(bool IsPlay,char *pLiveName)
{
	dbg("=======IsPlay=%d,pLiveName=%s\n",IsPlay,pLiveName);
}
#endif

void RtspSetCallback(rtsp_session_handle session,RtspCallbackStatus Callback, void *pObj)
{
	struct rtsp_session *s = (struct rtsp_session*) session;
	if(s)
		s->CallbackStatus=Callback;
}
