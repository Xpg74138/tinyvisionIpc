...
typedef void * rtsp_demo_handle;		//rtsp端口的监听句柄
typedef void * rtsp_session_handle;		//会话信息句柄（流名）
...
typedef void (*RtspCallbackStatus)(int IsPlay,char *pLiveName, void *pObj);
void RtspSetCallback(rtsp_session_handle session,RtspCallbackStatus Callback, void *pObj);		//状态回调函数用于连接后请求i帧或记录连接的客户端数
#ifdef __cplusplus
}
#endif
#endif
